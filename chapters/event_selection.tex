...
\ref{fig:z_rapidity_powheg}. The MC samples have had all of the
corrections discussed in \SEC~\ref{sec:corrections} applied, as well as the scale
factors discussed in \SEC~\ref{sec:scale_factors}. Even so, the \MADGRAPH
sample does not reproduce the distributions seen in data.

The effect of these disagreements is considered in
\SEC~\ref{four_vector_uncertainty}. The disagreement in the \Z \bosonpt
...
\input{tables/background_fraction.tex}
...
backgrounds in each \phistar bin are showing in \FIG~\ref{fig:qcd_phistar}, and
presented in \TAB~\ref{table:qcd_fits}.
...
\begin{figure}[!htbp]
    ...
    \caption[
        ...
        Estimate of the \QCDjets and \wjets background in each \phistar bin.
        Table~\ref{table:qcd_fits} presents the same information in numeric
        form.
    }
...
\dressed electrons. \TODO{\Born and \bare electrons are given in \APP Born, Bare
Appendix}.
