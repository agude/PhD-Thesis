...
\section{The \texorpdfstring{\Z}{Z} Boson}
...
decay mode is difficult to select as there are many hadronic jets in the each
event, and so \Ztoll decay modes are preferred. In this analysis we look at the
\Ztoee decay mode. A few common decay modes and their branching fraction are
listed in \TAB~\ref{table:z_decays}.

% table:z_decays
\input{tables/z_decay_modes.tex}

\subsection{\texorpdfstring{\Z}{Z} Boson Transverse Momentum}

The leptonic decays of the \Z are an especially good probe of QCD as neither
...
state radiation, which are discussed in \SEC~\ref{ssec:higher_order}. Low
...
\alphastrong is larger.

In addition aiding in the understanding of QCD processes, measuring the \Z
\bosonpt spectrum helps improve measurements of the \W mass. The \W \bosonpt
distribution is a source of uncertainty in the final measurement, and it can be
constrained by measuring the \Z \bosonpt spectrum.
...
In the low \bosonpt region---the region which contains most of the \Z
events---the measurement is dominated by the systematic uncertainties and the
experimental resolution. Theorists have proposed a new variable, \phistar, that
depends only on the direction of the leptons and not on their energy in order
to reduce the systematic uncertainties associated with the measurement
\cite{banfi_2011}. \phistar is correlated to $\bosonpt / \mll$, where $\mll$ is
the invariant mass of the two leptons, so it probes the same physics as
\bosonpt. This variable is less susceptible to detector resolution effects as
the energy and momentum resolutions of a detector are, in general, worse than
the position resolution. This is due to the fact that the position of leptons
is measured with a very fine-grained tracking device whereas the energy is
...
\ATLAS, one of the other experiments on the LHC, has measured \phistar at a
center-of-mass energy of $7 \TeV$ (\rootsseven) \cite{atlas_phistar}. \DZERO
has also measured \phistar at the Tevatron at \rootsTevatron
\cite{d0_phistar_2011}\cite{d0_phistar_2014}. This thesis presents the first
measurement of \phistar at \rootseight.
...
differential cross section measurement. A cross section is, at its heart, a
probability measurement. The cross section of a process, $\Z$ production for
...
where $\bosonptk$ is a range of \bosonptk, $N_{\Z_{\bosonptk}}$ is the number
of \Z events that fall within that range, and $\Delta \bosonptk$ is size of
that range. This measurement is still dependent on the luminosity, which can be
difficult to accurately measure. This dependency can be removed by dividing by
...
it is important to understand how they are produced in the Large Hadron
Collider (LHC).
...
quarks as the energy of the proton is much higher than its own binding energy
($\approx 1 \GeV$). Instead, it is described by a parton model---a model in
which there are three valence quarks, but also numerous gluons and
...
This fraction is given by the Bjorken \BjorkenX{i} variable, defined as
...
$\approx 1 \GeV$, is transverse to the beamline due to the motion of the
...
Feynman diagram of this process is shown in \FIG~\ref{fig:feyn_s_channel}. As
...
\ref{fig:feyn_qbarq_to_zg}, contains initial state radiation (ISR). These
diagrams have a gluon radiating from the incoming quark. This causes the quark
to gain momentum equal and opposite to that of the gluon. The quark will then
