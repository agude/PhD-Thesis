...
\TODO{
    This section should be expanded. We need to discuss the matrix level
    generators (\MADGRAPH and \POWHEG) and the code that does the hadronization
    (\PYTHIA).\\
    The first few do the "black board level" math, while \PYTHIA figures out
    what that means for the proton that you just ripped a gluon or quark out
    of.\\
    They have different implications for the boson $q_{T}$ and hence
    \phistar.\\
    Jeremy suggests looking at the \PYTHIA manual.
}
...
\TODO{All of the MC samples use \PYTHIAsix for hadronization except for the di-boson
samples. \Tauola is used for tau decays in all of the samples except for \ttbar
and \POWHEG $\text{Drell-Yan} \to \ee$ MC samples \cite{was_2007}.}

\TODO{Jeremy thinks I should add Hadronizers and tunes to this table, but
they are very consistant, maybe I can just write them in the text?}
