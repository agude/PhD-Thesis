...
parameters used in \PYTHIAsix are tuned in the MC samples used in this
analysis.

\subsection{Monte Carlo Datasets}
\label{ssec:monte_carlo}

All the MC samples used in this analysis were centrally-generated by the CMS
collaboration except for two \POWHEG \DYtoee samples which we generated. A
\DYtoll signal sample and a $\ttbar\text{+jets}$ background sample were
generated with \MADGRAPH. Diboson ($\Z\Z$, $\W\Z$, $\W\W$) background samples
were generated with \PYTHIAsix . Background samples consisting of $\tbar \W$,
$t \W$, and $\DYtotautau$ were generated using \POWHEG . A secondary signal MC
was also generated with \POWHEG. The details of these samples are listed in
\cref{table:mc}.
...
In addition to the samples list, the \POWHEG \DYtoee sample had two modified
versions produced. These two samples used \PYTHIAeight for the hadronization
step with new tunes, \TunePPfive and \TunePPfourteen.
