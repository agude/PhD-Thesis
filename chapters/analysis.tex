...
\FIGS~\ref{fig:madgraph_uncert_abs} and \ref{fig:powheg_uncert_abs} show the
...
in the normalized \phistar cross section in data, while
\FIGS~\ref{fig:madgraph_uncert_norm} and \ref{fig:powheg_uncert_norm} show the
uncertainty on the normalized \phistar cross section in \MADGRAPH and \POWHEG
MC respectively. These values are presented in tables in
...
% fig:sys_uncert_norm
\input{packaged_figures/data_uncertainty_normalized.tex}

% fig:madgraph_uncert_norm
\input{packaged_figures/madgraph_uncertainty_normalized.tex}

% fig:powheg_uncert_norm
\input{packaged_figures/powheg_uncertainty_normalized.tex}
...
\SEC~\ref{sec:acceptance}. Both an absolute and a normalized cross section
measurement are made.

The data was unfolded using the generated \phistar distribution from the
\MADGRAPH MC signal sample. This data distribution is compared to the
distributions from both the \MADGRAPH MC signal sample and the \POWHEG MC
signal sample. Additional results using \POWHEG to perform the unfolding are
presented in \APP~\ref{app:powheg_analysis}.

\subsection{Absolute Differential Cross Section}
\label{ssec:results_abs}

The absolute differential cross section measurement is shown in
\FIG~\ref{fig:results_abs} and given in tabular form in
...
The normalized differential cross section measurement is shown in
\FIG~\ref{fig:results_norm} and given in tabular form in
\TAB~\ref{tab:results_norm}.
The lower plot in \FIG~\ref{fig:results_norm} is
...
\input{packaged_figures/results_norm.tex}
% tab:results_norm
\input{tables/results_normalized.tex}
